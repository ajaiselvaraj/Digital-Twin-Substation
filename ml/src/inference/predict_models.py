...
class MockLSTM:
    def predict(self, seq, verbose=0):
        # seq shape is (1, 20, 1) or similar.
        val = float(seq[0][-1][0]) if len(seq.shape) == 3 else 70.0
        return np.array([[val]])

class MockXGBoost:
    def predict(self, features):
        # features shape is (1, N).
        return np.array([0.15])

class MockIsolationForest:
    def predict(self, features):
        # return a 1D numpy array with 1 (inlier)
        return np.array([1])
...
def load_models(component_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logging.getLogger("ml-service").warning(
            f"Failed to load real ML models for {component_name}: {e}. "
            "Using Mock model fallback (predictions will use heuristic fallbacks)."
        )
        return {
            "lstm": MockLSTM(),
            "xgb": MockXGBoost(),
